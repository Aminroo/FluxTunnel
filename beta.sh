...
# ─────────────────────────────────────────────────────
#  Proxy nc helper — سازگار با همه اوبونتوها
# ─────────────────────────────────────────────────────

# برمی‌گردونه بهترین دستور nc/ncat موجود برای SOCKS5 proxy
# خروجی: رشته‌ای که جلوش %h %p اضافه میشه
_proxy_nc_cmd() {
    local addr="$1"   # host:port
    local ph="${addr%:*}" pp="${addr##*:}"

    if command -v ncat &>/dev/null; then
        # ncat از nmap — روی همه اوبونتوها کار می‌کنه (20/22/24)
        echo "ncat --proxy ${ph}:${pp} --proxy-type socks5"
    elif command -v nc &>/dev/null && nc --help 2>&1 | grep -q -- '-x'; then
        # nc با پشتیبانی از -x (netcat-openbsd روی اوبونتو 24)
        echo "nc -x ${ph}:${pp} -X 5"
    else
        # fallback — nc بدون پشتیبانی proxy (بعید به کار بیاد ولی نمی‌شکنه)
        warn "No suitable proxy-capable nc/ncat found — proxy may not work"
        echo "nc -x ${ph}:${pp} -X 5"
    fi
}
...
fix_gateway_ports() {
    ...
    if ! sshd -t 2>/dev/null; then
        warn "sshd_config has existing syntax errors — fix manually before proceeding"
    fi
    ...
    if ! sshd -t 2>/dev/null; then
        err "sshd_config syntax error after edit — restoring backup"
        cp "${SSHD}.fluxtunnel.bak" "$SSHD"
        return 1
    fi
...
# ─────────────────────────────────────────────────────
#  Proxy checker — تست واقعی اتصال از طریق SOCKS5
# ─────────────────────────────────────────────────────

check_proxy() {
    local proxy_addr="$1"   # host:port
    local ph="${proxy_addr%:*}"
    local pp="${proxy_addr##*:}"

    echo ""
    step "Checking proxy ${proxy_addr}..."

    if command -v nc &>/dev/null; then
        if ! nc -z -w 3 "$ph" "$pp" 2>/dev/null; then
            err "Cannot reach proxy at ${ph}:${pp} (port closed or unreachable)"
            return 1
        fi
        ok "Proxy port ${pp} is reachable"
    fi
    ...
    if command -v curl &>/dev/null; then
        if curl -s --max-time 8 \
            ...
            -o /dev/null \
            "http://connectivity-check.ubuntu.com" 2>/dev/null; then
            ok "Proxy is working — outbound connection successful"
            return 0
        fi

        if command -v ncat &>/dev/null; then
            if ncat --proxy "${ph}:${pp}" --proxy-type socks5 -z -w 5 "$test_host" "$test_port" 2>/dev/null; then
                ok "Proxy is working (ncat test passed)"
                return 0
            fi
        elif command -v nc &>/dev/null && nc --help 2>&1 | grep -q -- '-x'; then
            if nc -x "${ph}:${pp}" -X 5 -z -w 5 "$test_host" "$test_port" 2>/dev/null; then
                ok "Proxy is working (nc test passed)"
                return 0
            fi
        fi

        err "Proxy is not forwarding traffic — connection test failed"
        info "Make sure your SOCKS5 proxy is running and allows outbound connections"
        return 1
        ...
        if ncat --proxy "${ph}:${pp}" --proxy-type socks5 -z -w 5 "$test_host" "$test_port" 2>/dev/null; then
            ...
            err "Proxy is not forwarding traffic"
            return 1
        fi
    elif command -v nc &>/dev/null && nc --help 2>&1 | grep -q -- '-x'; then
        if nc -x "${ph}:${pp}" -X 5 -z -w 5 "$test_host" "$test_port" 2>/dev/null; then
        ...
        warn "Cannot fully test proxy (curl/ncat/nc not available) — assuming it works"
        return 0
    fi
}
...
ask_proxy() {
    ...
    if [[ "$PROXY_CHOICE" == "1" ]]; then
        ...
        # بررسی proxy قبل از ادامه
        if ! check_proxy "$PROXY_ADDR"; then
            echo ""
            read -rp "  Proxy check failed. Continue anyway? [y/N]: " force
            [[ "${force,,}" != "y" ]] && { err "Aborted."; exit 1; }
            warn "Continuing with unverified proxy..."
        fi

        # ساخت ProxyCommand با helper سازگار با همه اوبونتوها
        PROXY_CMD="ProxyCommand=$(_proxy_nc_cmd "${ph}:${pp}") %h %p"
...
# ─────────────────────────────────────────────────────
#  Install deps — با پشتیبانی از proxy و نصب ncat
# ─────────────────────────────────────────────────────
...
install_deps() {
    ...
    # ncat (از nmap) — سازگار با اوبونتو 20/22/24، جایگزین nc -x
    if ! command -v ncat &>/dev/null; then
        pkgs+=(ncat)
    fi
    ...
    # nc به عنوان fallback اگه ncat نصب نشد
    ...
    if [[ ${#pkgs[@]} -gt 0 ]]; then
        ...
        if command -v apt-get &>/dev/null; then
            eval apt-get install -y -q "${pkgs[@]}" ${APT_PROXY_ARGS:-} \
                -o Dpkg::Progress-Fancy=0 \
                -o APT::Status-Fd=-1 \
                2>/dev/null | grep -E "^(Setting up|Unpacking)" | \
                while IFS= read -r line; do
                    info "$line"
                done
            for pkg in "${pkgs[@]}"; do
                local bin="$pkg"
                command -v "$bin" &>/dev/null && ok "$pkg installed" || warn "$pkg may have failed"
            done
    ...
    # نمایش وضعیت نهایی ابزار proxy
    if command -v ncat &>/dev/null; then
        ok "ncat: $(ncat --version 2>&1 | head -1)  ${DIM}(proxy tool)${NC}"
    elif command -v nc &>/dev/null && nc --help 2>&1 | grep -q -- '-x'; then
        ok "nc: proxy-capable (ubuntu 24 / BSD)"
    else
        warn "No proxy-capable nc/ncat — SSH proxy may not work"
    fi

    if command -v autossh &>/dev/null; then
        ok "autossh: $(autossh -V 2>&1 | head -1)"
    else
        warn "autossh not available — will use ssh fallback"
    fi
}
...
wait_for_connection() {
    ...
    step "Testing SSH connection to ${RH}:${SP}..."
    local args=(); [[ -n "$PC" ]] && args+=(-o "$PC")
    args+=(-o "StrictHostKeyChecking=accept-new" -o "ConnectTimeout=8" -o "BatchMode=yes" -p "$SP")

    for (( i=1; i<=10; i++ )); do
        if ssh "${args[@]}" "${RU}@${RH}" "exit" 2>/dev/null; then
            ok "SSH connection OK"
            return 0
        fi
        [[ $i -lt 10 ]] && { printf "  Attempt %d/10...\r" "$i"; sleep 2; }
    done
    echo ""
    err "Cannot connect to ${RH}:${SP} after 10 attempts"
    return 1
}
...
setup_client() {
    ...
    for (( i=1; i<=TUNNEL_COUNT; i++ )); do
        ...
        local def=$(( 20000 + i - 1 ))
        read -rp "  Port (same on VPS and local) [default ${def}]: " TP
        TP=${TP:-$def}
        LP="$TP"
        ENTRIES+=("${i}|${REMOTE_USER}|${REMOTE_HOST}|${SSH_PORT}|${TP}|${LP}|${PROXY_CMD}")
        ok "Tunnel #${i}: port ${TP} (local → VPS)"
    done
...
list_tunnels() {
    ...
    hr
    printf "  %-4s %-22s %-10s %-13s %-8s %-6s %s\n" \
        "#" "Remote Host" "SSH Port" "Port" "Proxy" "PW" "Status"
    hr
    ...
    while IFS='|' read -r ID RU RH SP TP LP PC; do
        ...
        printf "  %-4s %-22s %-10s %-13s %-8s " "$i" "$RH" "$SP" "$TP" "$proxy"
        echo -e "${pw_st}  ${svc_st}"
        (( i++ ))
    done < "$TUNNELS_FILE"
    echo ""
...
edit_tunnel() {
    ...
    while IFS='|' read -r ID RU RH SP TP LP PC; do
        ...
        if [[ "$i" -eq "$N" ]]; then
            ...
            read -rp "  Port         [${TP}]: " v; TP=${v:-$TP}; LP="$TP"
            ...
            if   [[ "$NP" == "none" ]]; then PC=""
                ...
                # استفاده از helper سازگار با همه اوبونتوها
                PC="ProxyCommand=$(_proxy_nc_cmd "${NP%:*}:${NP##*:}") %h %p"
            fi
...
uninstall_all() {
    ...
    rm -f  "${CONFIG_DIR}"/ssh_t*.conf 2>/dev/null && ok "Temp SSH configs removed"
    rm -rf "$CONFIG_DIR"    && ok "Config dir removed"
    systemctl daemon-reload
    echo ""; ok "Uninstall complete!"; echo ""; exit 0
}

# ─────────────────────────────────────────────────────
#  Tunnel runner script
#  نکته: _proxy_nc_cmd اینجا دوباره تعریف میشه چون این اسکریپت
#  مستقل از setup.sh اجرا میشه (توسط systemd)
# ─────────────────────────────────────────────────────
...
run_tunnel() {
    ...
    if command -v autossh &>/dev/null; then
        ...
        while true; do
            ...
            log "[T${ID}] autossh exited (code=$?) — retry in 3s..."
            sleep 3
        done
    else
        log "[T${ID}] Starting ssh fallback → ${RH}:${TP}"
        local backoff=5
        while true; do
            log "[T${ID}] Connecting..."
            if _ssh_once; then backoff=5; fi
        ...
        case "$CHOICE" in
            ...
            8)
                ...
                if [[ "$mode" == "client" ]]; then
                    ...
                    warn "Not configured yet"; sleep 1
                fi
                ;;
            9)  self_update ;;
            10) uninstall_all ;;
